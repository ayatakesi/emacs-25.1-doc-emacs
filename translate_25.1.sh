...
s/\@section Lax Matching During Searching$/\@section 検索中のLaxマッチング/;
...
s/\@subsection Replace Commands and Lax Matches$/\@subsection Replace 置換コマンドとLaxマッチ/;
s/\@subsection Query Replace$/\@subsection 問い合わせつき置換/;
s/\@section Other Search-and-Loop Commands$/\@section その他の検索およびループコマンド/;
s/\@section Tailoring Search to Your Needs$/\@section 必要に応じて検索を調整する/;' -i.bak search.texi
...
s/\@section Deleting and Resizing Windows$/\@section ウィンドウの削除とリサイズ/;
...
s/\* Temporary Displays::       Displaying non-editable buffers\./* Temporary Displays::       編集不可バッファーの表示/;
s/\@subsection Displaying non-editable buffers.$/\@subsection 編集不可バッファーの表示/;
...
s/\@subsubsection Pulling\/Pushing Changes into\/from a Branch/\@subsubsection ブランチへ\/からの変更のpull\/push/;
...
s/\@section Find Identifier References$/\@section 識別子のリファレンスを探す/;
s/\@subsection Find Identifiers$/\@subsection 識別子を探す/;
s/\@subsubsection Looking Up Identifiers$/\@subsubsection 識別子のルックアップ/;
s/\@subsubsection Commands Available in the \@file\{\*xref\*\} Buffer$/\@subsubsection \@file\{\*xref\*\}バッファーで利用可能なコマンド/;
s/\@subsubsection Searching and Replacing with Identifiers$/\@subsubsection 識別子の検索と置換/;
s/\@subsubsection Identifier Inquiries$/\@subsubsection 識別子の照会/;
s/\@subsection Tags Tables$/\@subsection タグテーブル/;
s/\@subsubsection Source File Tag Syntax$/\@subsubsection ソースファイルタグの構文/;
s/\@subsubsection Creating Tags Tables$/\@subsubsection タグテーブルの作成/;
...
s/\@subsection Copyright Assignment$/\@subsection 著作権の譲渡/;
...
perl -pe '
s/\@appendix Emacs 24 Antinews$/\@appendix Emacs 24 アンチニュース/;' -i.bak anti.texi
...
s/\@section Subprocesses on Windows 9X\/ME and Windows NT\/2K\/XP\/Vista\/7\/8\/10$/\@section Windows 9X\/MEおよびWindows NT\/2000\/XP\/Vista\/7\/8\/10でのサブプロセス/;
...
s/\@section Miscellaneous Windows-specific features$/\@section その他のWindows固有の機能/;' -i.bak msdos.texi 
...
s/\@subsection Subprocesses on MS-DOS$/\@subsection MS-DOSでのサブプロセス/;' -i.bak msdos-xtra.texi 
