...
perl -pe 's/\@section Point$/\@section ポイント/' -i screen-ja.texi
perl -pe 's/\@section Keys$/\@section キー/' -i commands-ja.texi
perl -pe 's/\@section Entering Emacs$/\@section Emacsの起動/' -i entering-ja.texi
perl -pe \
...
perl -pe 's/\@section Shift Selection$/\@section シフト選択/' -i mark-ja.texi
perl -pe \
...
's/\@section Customization of Display$/\@section ディスプレーのカスタマイズ/;' -i display-ja.texi
perl -pe \
...
perl -pe 's/\@section Undo$/\@section Undo(取り消し)/' -i fixit-ja.texi
perl -pe 's/\@chapter Keyboard Macros$/\@chapter キーボードマクロ/' -i kmacro-ja.texi
perl -pe 's/\@subsection Auto Reverting the Buffer Menu$/\@subsection Buffer Menuの自動リバート/' -i arevert-xtra-ja.texi
perl -pe \
...
's/\@section Window Dividers$/\@section ウィンドウdivider/;'\
...
perl -pe 's/\@subsection Fortran Comments$/\@subsection Fortranのコメント/' -i fortran-xtra-ja.texi
perl -pe \
...
's/\@subsubsection Looking Up Identifiers$/\@subsubsection 識別子の照合/;'\
's/\@subsection Format of ChangeLog$/\@subsection ChangeLogの書式/;'\
's/\@subsection Tags Tables$/\@subsection tagsテーブル/;'\
's/\@subsubsection Etags Regexps$/\@subsubsection etagsのregexp/;'\
's/\@section Emacs Development Environment$/\@section Emacs開発環境/;'  -i maintaining-ja.texi
perl -pe 's/\@section Advanced VC Usage$/\@section VCの上級の使用法/;' -i vc-xtra-ja.texi
perl -pe \
...
's/\@section Summing Time Intervals$/\@section 時間間隔の加算/;' -i calendar-ja.texi
perl -pe \
...
's/\@section Embedded WebKit Widgets$/\@section 埋め込みWebKitウィジェット/;'\
 -i misc-ja.texi
perl -pe 's/\@section Package Installation$/\@section パッケージのインストール/' -i package-ja.texi
perl -pe \
...
's/\@subsection Understanding Bug Reporting$/\@subsection バグレポートの理解/;' -i trouble-ja.texi
perl -pe \
