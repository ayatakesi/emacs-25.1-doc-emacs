...
%.texi:
	...
	then \
		...
		case "$@" in \
		"screen.texi" ) perl -pe 's/\@section Point$$/\@section ポイント/' -i $${JA_TEXI} ;; \
		"commands.texi" ) perl -pe 's/\@section Keys$$/\@section キー/' -i $${JA_TEXI} ;; \
		"entering.texi" ) perl -pe 's/\@section Entering Emacs$$/\@section Emacsの起動/' -i $${JA_TEXI} ;; \
		"basic.texi" ) perl -pe \
's/\@section Inserting Text/\@section テキストの挿入/;'\
's/\@section Blank Lines/\@section 空行/;'\
's/\@section Continuation Lines/\@section 継続行/;' -i $${JA_TEXI} ;; \
		"mini.texi" ) perl -pe \
...
's/\@section Yes or No Prompts/\@section Yes or No プロンプト/;' -i $${JA_TEXI} ;; \
		"help.texi" ) perl -pe \
's/\@chapter Help$$/\@chapter ヘルプ/;'\
's/\@section Help Summary$$/\@section ヘルプの概要/;'\
's/\@section Apropos$$/\@section Apropos(適切な)/;'\
's/\@section Help Files$$/\@section ヘルプファイル/;' -i $${JA_TEXI} ;; \
		"mark.texi" ) perl -pe 's/\@section Shift Selection$$/\@section シフト選択/' -i $${JA_TEXI} ;; \
...
's/\@section CUA Bindings$$/\@section CUAバインド/;' -i $${JA_TEXI} ;; \
		"regs.texi" ) perl -pe \
's/\@chapter Registers$$/\@chapter レジスター/;'\
's/\@section Keyboard Macro Registers/\@section キーボードマクロのレジスター/;'\
's/\@section Bookmarks/\@section ブックマーク/;' -i $${JA_TEXI} ;; \
...
's/\@section Customization of Display$$/\@section ディスプレーのカスタマイズ/;' -i $${JA_TEXI} ;; \
		"search.texi" ) perl -pe \
...
's/\@subsection Query Replace$$/\@subsection 問い合わせつき置換/;' -i $${JA_TEXI} ;; \
		"fixit.texi" ) perl -pe 's/\@section Undo$$/\@section Undo(取り消し)/' -i $${JA_TEXI} ;; \
		"kmacro.texi" ) perl -pe 's/\@chapter Keyboard Macros$$/\@chapter キーボードマクロ/' -i $${JA_TEXI} ;; \
		"arevert-xtra.texi" ) perl -pe 's/\@subsection Auto Reverting the Buffer Menu$$/\@subsection Buffer Menuの自動リバート/' -i $${JA_TEXI} ;; \
		"files.texi" ) perl -pe \
...
's/\@section Filesets$$/\@section ファイルセット/;' -i $${JA_TEXI} ;; \
		"buffers.texi" ) perl -pe 's/\@section Indirect Buffers$$/\@section インダイレクトバッファー/' -i $${JA_TEXI} ;; \
		"frames.texi" ) perl -pe \
...
's/\@section Non-Window Terminals$$/\@section 非ウィンドウ端末/;' -i $${JA_TEXI} ;; \
		"mule.texi" ) perl -pe \
...
's/\@section Bidirectional Editing$$/\@section 双方向の編集/;' -i $${JA_TEXI} ;; \
		"modes.texi" ) perl -pe \
's/\@section Major Modes$$/\@section メジャーモード/;'\
's/\@section Minor Modes$$/\@section マイナーモード/;' -i $${JA_TEXI} ;; \
		"indent.texi" ) perl -pe \
's/\@chapter Indentation$$/\@chapter インデント/;'\
's/\@section Indentation Commands$$/\@section インデントコマンド/;'\
's/\@section Tab Stops$$/\@section タブストップ/;' -i $${JA_TEXI} ;; \
		"text.texi" ) perl -pe \
...
's/\@subsection Table Rows and Columns$$/\@subsection テーブルの行と列/;' -i $${JA_TEXI} ;; \
...
's/\@section Asm Mode$$/\@section Asmモード/;' -i $${JA_TEXI} ;; \
		"fortran-xtra.texi" ) perl -pe 's/\@subsection Fortran Comments$$/\@subsection Fortranのコメント/' -i $${JA_TEXI} ;; \
...
's/\@subsubsection Multithreaded Debugging$$/\@subsubsection マルチスレッドのデバッグ/;' -i $${JA_TEXI} ;; \
...
's/\@section Emacs Development Environment$$/\@section Emacs開発環境/;'  -i $${JA_TEXI} ;; \
		"vc-xtra.texi" ) perl -pe 's/\@section Advanced VC Usage$$/\@section VCの上級の使用法/;' -i $${JA_TEXI} ;; \
...
's/\@subsection Customizing VC$$/\@subsection VCのカスタマイズ/;' -i $${JA_TEXI} ;; \
...
's/\@subsection Fine Points of Emerge$$/\@subsection Emergeの細かい注意点/;' -i $${JA_TEXI} ;; \
...
's/\@section Saving Abbrevs$$/\@section abbrevの保存/;' -i $${JA_TEXI} ;; \
		"dired.texi" ) perl -pe \
...
's/\@section Hiding Subdirectories$$/\@section サブディレクトリーを隠す/;' -i $${JA_TEXI} ;; \
...
's/\@section Summing Time Intervals$$/\@section 時間間隔の加算/;' -i $${JA_TEXI} ;; \
...
's/\@subsection Fancy Diary Display$$/\@subsection Fancy Diary表示/;' -i $${JA_TEXI} ;; \
...
's/\@section Mail Amusements$$/\@section アミューズメント/;' -i $${JA_TEXI} ;; \
		"rmail.texi" ) perl -pe \
's/\@section Rmail Attributes$$/\@section Rmailの属性/;'\
's/\@section \@code\{movemail\} program$$/\@section \@code\{movemail\}プログラム/;' -i $${JA_TEXI} ;; \
		"misc.texi" ) perl -pe \
's/\@section Gnus$$/\@section Gnus/;'\
's/\@section Host Security$$/\@section ホストのセキュリティ/;'\
's/\@section Network Security$$/\@section ネットワークのセキュリティ/;'\
...
-i $${JA_TEXI} ;; \
		"package.texi" ) perl -pe 's/\@section Package Installation$$/\@section パッケージのインストール/' -i $${JA_TEXI} ;; \
		"custom.texi" ) perl -pe \
...
's/\@subsection Modifier Keys$$/\@subsection 修飾キー/;' -i $${JA_TEXI} ;; \
		"trouble.texi" ) perl -pe \
's/\@subsection Emergency Escape$$/\@subsection 緊急エスケープ/;'\
's/\@subsection Understanding Bug Reporting$$/\@subsection バグレポートの理解/;' -i $${JA_TEXI} ;; \
		"cmdargs.texi" ) perl -pe \
's/\@appendixsec Action Arguments$$/\@appendixsec 動作引数/;'\
's/\@appendixsec Initial Options$$/\@appendixsec 初期化オプション/;'\
's/\@appendixsubsec General Variables$$/\@appendixsubsec 一般的な変数/;' -i $${JA_TEXI} ;; \
...
's/\@appendixsubsec GTK styles$$/\@appendixsubsec GTKスタイル/;' -i $${JA_TEXI} ;; \
		"macos.texi" ) perl -pe \
's/\@section Mac \/ GNUstep Customization$$/\@section Mac\/GNUstepでのカスタマイズ/;'\
's/\@section GNUstep Support$$/\@section GNUstepにたいするサポート/;' -i $${JA_TEXI} ;; \
		"emacs-xtra.texi" ) perl -pe \
's/\@settitle Specialized Emacs Features$$/\@settitle Specialized Emacs Features\(Japanese Translation\)/;'\
's/\@title Specialized Emacs Features$$/\@title Specialized Emacs Features\(Japanese Translation\)/;' -i $${JA_TEXI} ;; \
		esac \
	else \
